...
def load_significant_frames(video_path, significant_indices, x_cord, y_cord):
...
def cfar_fast(
    ...
    # fmt: on
    return window_mean + bias, window_mean - bias
...
### only for sizetunning data
def load_annotation(excel, set_no):
    if set_no == 1:
        excel_path = excel
        ...
        file = excel
        cols = np.split(np.arange(4,13,1), 3)

        ex = pd.read_excel(file, usecols = cols[set_no-2], header = 1).drop(np.append(np.arange(49,49,1), 0)).reset_index().drop(["index"], axis=1)
        ex.columns = ['azimuth', 'is_pc', 'is_strike']

        is_pc = ex['is_pc'].values
        is_strike = ex['is_strike'].values
        azimuth = ex['azimuth'].values
        pc_only = is_pc - is_strike
        strike_only = is_strike
        pc_only = np.nan_to_num(pc_only)
        strike_only = np.nan_to_num(strike_only)
        ...
        df_trial = pd.DataFrame({
            "t_start" : np.arange(48) * 15 + 30,
            "t_end" : np.arange(48) * 15 + 32
        })

    return is_pc, pc_only, strike_only, df_trial